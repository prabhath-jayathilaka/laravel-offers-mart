<header class="">
    ...
    <nav class=" border-gray-300  px-2 sm:px-4 py-2.5  dark:bg-gray-800">
        ...
        <div class=" mt-2 container flex flex-wrap justify-between items-center mx-auto ">
            ...
            <a href="{{ route('not-set') }}"><x-application-logo class="w-3/4 h-full "></x-application-logo></a>
            ...
            <div class="hidden w-full md:block md:w-auto" id="mobile-menu">
                <ul class="flex flex-col mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-md md:font-bold md:text-lg">
                    <li>
                        <a href="{{ route('not-set') }}" class="block py-2 pr-4 pl-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white" aria-current="page">Home</a>
                    </li>
                    <li>
                        <a href="{{ route('shop') }}"
            ...
            @if (Route::has('login'))
                <div class=" px-6 py-4 order-2 w-1/3  flex justify-end  ">
                    ...
                    <ul class="flex flex-col mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-md md:font-medium">
                        ...
                        @auth
                            <li><a href="{{ route('register') }}" class="ml-4 text-md text-gray-700 dark:text-gray-500  underline">Dashboard</a></li>
                            <li>
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf

                                    <button class="text-md text-gray-700 dark:text-gray-500 underline" :href="route('logout')"
                        ...
                        @else
                            <li><a href="{{ route('login') }}" class="text-md text-gray-700 dark:text-gray-500 underline">Log in</a></li>

                            @if (Route::has('register'))
                                <li><a href="{{ route('register') }}" class="ml-4 text-md text-gray-700 dark:text-gray-500 underline">Register</a> </li>
                            @endif
                        @endauth
